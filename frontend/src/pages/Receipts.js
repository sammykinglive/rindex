        ...
        <form onSubmit={onSubmit}>
          <div className="modal-body">
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <input className="form-control" placeholder="e.g. GRN-001" value={form.grn_number} onChange={e => setForm({...form, grn_number: e.target.value})} required />
              </div>
              <div className="form-group" style={{ gridColumn: '1/-1' }}>
                <label className="form-label">Supplier Name *</label>
                <input className="form-control" placeholder="e.g. Agro Suppliers Ltd" value={form.supplier_name} onChange={e => setForm({...form, supplier_name: e.target.value})} required />
              </div>
              <div className="form-group">
                <label className="form-label">Quantity (Bags) *</label>
                <input className="form-control" type="number" min="1" placeholder="e.g. 1000" value={form.quantity} onChange={e => setForm({...form, quantity: e.target.value})} required />
              </div>
              <div className="form-group">
                <label className="form-label">Unit Cost (GHS/Bag) *</label>
                <input className="form-control" type="number" step="0.01" placeholder="e.g. 280" value={form.unit_cost} onChange={e => setForm({...form, unit_cost: e.target.value})} required />
              </div>
              {total > 0 && (
              ...
              <div className="form-group">
                ...
                <input className="form-control" placeholder="e.g. DN-2026-001" value={form.delivery_note} onChange={e => setForm({...form, delivery_note: e.target.value})} />
              </div>
              <div className="form-group">
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn btn-success" disabled={loading}>
              {loading ? <><span className="spin">⟳</span> Saving…</> : 'Save Receipt'}
            </button>
          </div>
        </form>
      </div>
...
export default function Receipts() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || 'Error saving receipt.');
  ...
  const conditionBadge = { Good: 'badge-green', Damaged: 'badge-red', 'Partial Damage': 'badge-gold' };
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Record every delivery of maize bags into the warehouse</div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 20 }}>
        {[
          ['Total Bags Received', fmt.number(totals.total_bags) + ' bags', 'var(--green-light)', 'var(--green)'],
        ...
        <div className="table-wrap">
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <p>Click "Record Delivery" to add your first stock entry.</p>
            </div>
          ) : (
            <table>
              <thead>
                <tr>
                  <th>Date</th><th>GRN No.</th><th>Supplier</th><th>Qty (Bags)</th>
                  <th>Unit Cost</th><th>Total Cost</th><th>Del. Note</th><th>Condition</th><th>Recorded By</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {receipts.map(r => (
                  <tr key={r.id}>
                    ...
                    <td style={{ fontWeight: 700, color: 'var(--green)' }}>{fmt.number(r.quantity)}</td>
                    <td>{fmt.currency(r.unit_cost)}</td>
                    <td style={{ fontWeight: 700 }}>{fmt.currency(r.total_cost)}</td>
                    <td style={{ color: 'var(--text-muted)', fontSize: 12 }}>{r.delivery_note || '—'}</td>
                    <td><span className={`badge ${conditionBadge[r.condition] || 'badge-blue'}`}>{r.condition}</span></td>
                    <td style={{ color: 'var(--text-muted)', fontSize: 12 }}>{r.created_by_name || '—'}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => openEdit(r)} title="Edit"><Pencil size={13} /></button>
                        <button className="btn btn-ghost btn-sm" onClick={() => handleDelete(r.id)} title="Delete" style={{ color: 'var(--red)' }}><Trash2 size={13} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <tfoot>
                <tr className="tfoot-row">
                  ...
                  <td style={{ fontWeight: 800 }}>{fmt.number(receipts.reduce((s, r) => s + r.quantity, 0))} bags</td>
                  <td></td>
                  <td style={{ fontWeight: 800 }}>{fmt.currency(receipts.reduce((s, r) => s + r.total_cost, 0))}</td>
                  <td colSpan={4}></td>
                </tr>
              </tfoot>
            </table>
