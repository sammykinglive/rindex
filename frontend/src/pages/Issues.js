...
import { printInvoice } from '../utils/exportPdf';
...
export default function Issues() {
  ...
  function handlePrintInvoice(issue) {
    printInvoice(issue, settings);
  }
  ...
  async function getExportData() {
    ...
    const exportIssues = res.data.issues;
    return {
      title: 'Stock Issues Report',
      sheetName: 'Stock Issues',
      headers: ['Date','Invoice No.','Customer Name','Qty (Bags)','Selling Price (GHS)','Total Sales (GHS)','Payment Method','Payment Status','Remarks'],
      columnWidths: [14,16,24,12,18,18,16,14,24],
      rows: exportIssues.map(r => [
    ...
    <div>
      <div className="page-header">
        ...
        <div style={{display:'flex', gap:8, flexWrap:'wrap'}}>
          <ExportButtons onGetData={getExportData} filename="stock_issues" />
      ...
      <div style={{display:'flex', gap:16, marginBottom:20, flexWrap:'wrap'}}>
        {[
          ...
          <div key={label} style={{background:bg, padding:'14px 20px', borderRadius:'var(--radius)', flex:1, minWidth:160}}>
      ...
      <div className="card">
        <div className="table-wrap">
          {loading ? (
            <div style={{textAlign:'center',padding:40}}><div style={{width:32,height:32,border:'3px solid var(--primary-pale)',borderTopColor:'var(--primary)',borderRadius:'50%',animation:'spin 0.8s linear infinite',margin:'0 auto'}}/></div>
          ) : issues.length === 0 ? (
            <div className="empty-state"><div className="empty-state-icon">🛒</div><h3>No issues recorded yet</h3><p>Click "Record Sale" to add your first sale.</p></div>
          ) : (
            <table>
              <thead>
                <tr><th>Date</th><th>Invoice</th><th>Customer</th><th>Qty</th><th>Price/Bag</th><th>Total Sales</th><th>Payment</th><th>Status</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {issues.map(r => (
                  <tr key={r.id}>
                    <td style={{fontSize:12.5,color:'var(--text-muted)'}}>{fmt.date(r.date)}</td>
                    <td style={{fontFamily:'monospace',fontWeight:600,fontSize:13}}>{r.invoice_number}</td>
                    ...
                    <td>
                      <div style={{display:'flex',gap:4}}>
                        <button className="btn btn-ghost btn-sm" onClick={() => handlePrintInvoice(r)} title="Print Invoice" style={{color:'var(--primary)',padding:'5px 8px'}}>
                          <Printer size={13}/>
                        </button>
                        <button className="btn btn-ghost btn-sm" onClick={() => openEdit(r)} title="Edit" style={{padding:'5px 8px'}}><Pencil size={13}/></button>
                        <button className="btn btn-ghost btn-sm" onClick={() => handleDelete(r.id)} title="Delete" style={{color:'var(--red)',padding:'5px 8px'}}><Trash2 size={13}/></button>
                      </div>
                    </td>
                  </tr>
                ))}
