...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    let q = 'SELECT i.*, u.name as created_by_name, c.name as commodity_name, c.unit as commodity_unit FROM stock_issues i LEFT JOIN users u ON i.created_by = u.id LEFT JOIN commodities c ON i.commodity_id = c.id WHERE 1=1';
    ...
    const p = [], tp = [];
    if (from)         { q += ' AND i.date >= ?';            tq += ' AND date >= ?';              p.push(from);                  tp.push(from); }
    if (to)           { q += ' AND i.date <= ?';            tq += ' AND date <= ?';              p.push(to);                    tp.push(to); }
    if (customer)     { q += ' AND i.customer_name LIKE ?'; tq += ' AND customer_name LIKE ?';   p.push(`%${customer}%`);       tp.push(`%${customer}%`); }
    if (status)       { q += ' AND i.payment_status = ?';   tq += ' AND payment_status = ?';     p.push(status);                tp.push(status); }
    if (commodity_id) { q += ' AND i.commodity_id = ?';     tq += ' AND commodity_id = ?';       p.push(parseInt(commodity_id)); tp.push(parseInt(commodity_id)); }
    q += ' ORDER BY i.date DESC, i.id DESC';
    const issues = await all(q, p);
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await run(
      ...
      [commodity_id ? parseInt(commodity_id) : null, date, invoice_number, customer_name, parseInt(quantity), parseFloat(selling_price), total_sales, payment_method || 'Cash', payment_status || 'Paid', remarks || '', req.user.id]
    );
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    await run(
      ...
      [commodity_id ? parseInt(commodity_id) : null, date, invoice_number, customer_name, parseInt(quantity), parseFloat(selling_price), total_sales, payment_method, payment_status, remarks, req.params.id]
    );
